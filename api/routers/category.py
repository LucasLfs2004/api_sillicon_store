...
def create_category(name: str = Form(), brand_logo: UploadFile = File(...), brand_logo_black: UploadFile = File(...)):
    try:
        cursor = mysql_connection.cursor(dictionary=True)
        ...
        # cursor.execute(
        #     "INSERT INTO CATEGORY (id, name, brand_logo, brand_logo_black) VALUES (%s, %s %s, %s)",
        #     (id, name, filename_brand_logo)
        # )
        ...
        return {
            "name:": name,
            "brand_logo_path:": filename_brand_logo, 
            "brand_logo_black_path:": filename_brand_logo_black, 
        }
    except Exception as e:
