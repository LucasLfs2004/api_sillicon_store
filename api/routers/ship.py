...
async def set_ship_info(ship: ship_info_post, current_user: int = Depends(token.get_current_user)):
    try:
        cursor = mysql_connection.cursor(dictionary=True)
        cursor.execute(
            'SELECT * FROM ship_info WHERE id_person = %s', (current_user,))
        ship_infos_fetched = cursor.fetchall()

        print(ship_infos_fetched)
        ...
        if ship.principal_ship or not ship_infos_fetched:
            cursor.execute(
